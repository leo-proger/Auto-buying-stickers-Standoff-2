...
def get_button_click() -> Tuple[int, int]:
    ...
    print("\nВам нужно кликнуть на кнопку подтверждения покупки лота (вам придется купить скин, если хватит голды)")
...
def update_config(coords: Tuple[int, int]) -> None:
    ...
    config['BUY_BUTTON_COORDINATES'] = list(coords)

    with open('config.json', 'w', encoding='utf-8') as f:
        json.dump(config, f, indent=4, ensure_ascii=False)

    print(f"\nКоординаты {coords} кнопки подтверждения покупки лота успешно обновлены в config.json")


def main():
    try:
        print("Настройка координат кнопки подтверждения покупки лота")
